    ...
    def _parse_model(self, node_infos):
        ...
        for node in node_infos:
            ...
            if node.get('op_type', '') in self.supported_ops:
                ...
                print("%s not supported" % node.get('op_type', ''), node)
...
def get_pose_net_mf(jsn, params, input_node_ids=[0], output_node_ids=[576,581,586]):
    model = TorchIr(jsn, params, input_node_ids, output_node_ids)


if __name__ == '__main__':
    jsn = '/home/jiachao/JCLiu/projects_2020/moffett/toolchain_mf/frontendcompiler/outputs/centernet_resnetv1b_map31/IR_for_reconstruct_graph.json'
    params = '/home/jiachao/JCLiu/projects_2020/moffett/toolchain_mf/frontendcompiler/outputs/centernet_resnetv1b_map31/IR_for_reconstruct_params.npz'
    #jsn = '../frontendcompiler/outputs/centernet_resnetv1b_map31/IR_for_reconstruct_graph.json'
    #params = '../frontendcompiler/outputs/centernet_resnetv1b_map31/IR_for_reconstruct_params.npz'
    #model = TorchIr(jsn, params, input_node_ids = [0], output_node_ids = [576,581,586])
    model = TorchIr(jsn, params, input_node_ids = [0], output_node_ids = [568])
    #model.eval()
    #print(model)
    data = torch.ones((10,3,512,512), dtype=torch.float32).uniform_(0, 1)
    results = model(data)
    print(results.shape)
    '''
